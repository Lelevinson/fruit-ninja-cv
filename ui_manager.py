        ...
        if self.image:
            ...
            if self.hover:
                ...
                glow_surf = pygame.Surface(
                    (scaled_w + 10, scaled_h + 10), pygame.SRCALPHA
                )
                pygame.draw.rect(
                    glow_surf, (*WHITE, 30), glow_surf.get_rect(), border_radius=10
                )
...
class SceneManager:
    def __init__(self, width, height):
        ...
        self.btn_play = FrameButton(
            cx - 100, cy + 50, 200, 80, "PLAY", "GOTO_MODE", use_image="btn_play.png"
        )

        # Mode Select - Transparent with white borders
        self.btn_classic = FrameButton(
            cx - 220, cy + 20, 200, 60, "CLASSIC", "MODE_CLASSIC"
        )
        self.btn_survival = FrameButton(
            cx + 20, cy + 20, 200, 60, "SURVIVAL", "MODE_SURVIVAL"
        )
